import altair as alt
import streamlit as st
import pandas as pd
from filters import filter_dataset



def page2(data, countries):
    
    #define sidebar sliders
    select_country = st.sidebar.multiselect("Select the countries you want to visualize: ", countries, default = ["Spain","Germany","Greece","Italy", "Switzerland"])
    select_year = st.sidebar.slider(label = "Select the year", min_value=2010, max_value=2019, value=2019, step=1)
     
    #select data from the dataset
    source = filter_dataset(data, index = "EMP9_5", countries = select_country)

    #Define interaction (if you click on any of the three graphs the corresponding country is highlighted in all of them)
    click = alt.selection_multi(encodings = ['color'])

    st.markdown("## Gender Wage Gap")
    st.markdown("The gender wage gap is defined as the difference between male and female median wages divided by the male median wages. You can filter the countries you want to visualize and the year thanks to the slider on the left. You can pass the cursor on the map to visualize more information. If with to highlight one country you can click on its corresponding line.")
    #simple line chart
    line_chart = alt.Chart(source).mark_line(point=True).encode(
        x = alt.X("Year:O",title ="Year"),
        y = alt.Y("Value:Q",title = "Index"),
        color = alt.Color(
            "Country:N",
            legend = alt.Legend(title = "Country")
        ),
        tooltip=['Year', 'Country', 'Value'],
        opacity = alt.condition(click, alt.value(1), alt.value(0.2))   
    ).add_selection(
        click
    ).properties(
    width=600,
    height=300
    )
    
    #filter data for maternal leave
    maternal_leave = filter_dataset(data, index = "EMP18_MAT", countries = select_country, year = select_year)
    maternal_leave["icon"]="data:image/png;base64,"

    #filter data for paternal leave
    paternal_leave = filter_dataset(data, index = "EMP18_PAT", countries = select_country, year = select_year)
    paternal_leave["icon"]="data:image/png;base64,"

    bars1 = alt.Chart(maternal_leave).mark_bar().encode(
    y='Country:N',
    x='Value:Q',
    color=alt.Color("Country:N"),
    opacity = alt.condition(click, alt.value(1), alt.value(0.2)) ,
    tooltip=['Country', 'Value'] 
    ).add_selection(
        click
    ).properties(
        width=200,
        height=250
    )

    icons1 = alt.Chart(maternal_leave).mark_image(
        width = 30,
        height = 30
    ).encode(
        y='Country:N',
        x=alt.X('Value:Q', title = "Weeks of Maternal Leave"),
        url = 'icon',
        opacity = alt.condition(click, alt.value(1), alt.value(0.2)),
        tooltip=['Country', 'Value'] 
    ).properties(
        width=200,
        height=250
    )

    maternal = bars1 + icons1

    bars2 = alt.Chart(paternal_leave).mark_bar().encode(
        y='Country:N',
        x='Value:Q',
        color=alt.Color("Country:N", legend=alt.Legend(title="Country")),
        opacity = alt.condition(click, alt.value(1), alt.value(0.2)),
        tooltip=['Country', 'Value']   
    ).add_selection(
        click
    ).properties(
        width=200,
        height=250
    )

    icons2 = alt.Chart(paternal_leave).mark_image(
        width = 30,
        height = 30
    ).encode(
        y='Country:N',
        x=alt.X('Value:Q', title = "Weeks of Paternal Leave"),
        url = 'icon',
        opacity = alt.condition(click, alt.value(1), alt.value(0.2)),
        tooltip=['Country', 'Value']  
    ).properties(
        width=200,
        height=250
    )

    paternal = bars2+icons2

    leaves = alt.hconcat(
        maternal,paternal
    ).properties(
    title={
      "text": [" ","Maternity vs Paternity Leave"," "], 
      "align": "left",
      "fontSize":30,
      "fontWeight":"bold",
    }
    )

    graph = alt.vconcat(line_chart, leaves).configure_view(strokeWidth=0).configure_axis(grid=False, domain=False)
    st.altair_chart(graph,use_container_width=True)

    st.markdown("Maternity leave refers to the number of weeks of job-protected leave available for mothers just before and after childbirth. For countries where there is no separate legislation for maternity leave, the weeks of parental leave reserved for the exclusive use of mothers around childbirth are reported. \
        Paid father-specific leave refers to the number of paid weeks reserved for the exclusive use of fathers, including entitlements to paid paternity leave, 'father quotas' or periods of paid parental leave that can be used only by the father and cannot be transferred to the mother, and any weeks of paid sharable leave that must be taken by the father in order for the family to qualify for 'bonus' weeks of parental leave. \
        The goal in our opinion should be to give fathers and mothers the same possibilities in these terms, to allow women and men to dedicate an equal amount of time to new born childs. \
        Moreover, as fathers have an higher possibility to stay at home, it decreases the possibility that women will be penalized at work for a pregrancy.")
    
    st.markdown("\n Source: https://stats.oecd.org/")

